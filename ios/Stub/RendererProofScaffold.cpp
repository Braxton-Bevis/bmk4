// Device-only link closure for the bounded IW3 R/RB placeholder proof
// (main's simulator lane does not link libkisakrenderer, so it needs none
// of these definitions; BootScaffold keeps its own scaffolds there).
//
...
// The device app dead-strips with the Vulkan loader entry explicitly rooted
// (see ios/project.yml); any deferred-owner reference the stripper cannot
// prune must land here fail-closed, never as a silent success.
...
// into the device renderer archive.
r_globals_t rg{};
...
// graduates into the device archive.
